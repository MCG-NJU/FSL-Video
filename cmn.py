...
def train(data_loader_list, model, optimization, start_epoch, stop_epoch, params):    
    ...
    for epoch in range(start_epoch,stop_epoch):
        ...
        for i, (x,_ ) in enumerate(base_loader):
            ...
            logits = cmn(logits,model) # 10 x 2048
            # compute loss
            logits = logits.reshape(nway,sq,-1) # 5 x 2 x 2048
...
def cmn(p,model):
    ...
    # x = x.reshape(nway*sq*t, c, h, w)
    # p = self.feature.forward(x) # 80 x 2048
    
    p = p.reshape(nway * sq,t,p.shape[-1]) # 10 x 8 x 2048
    ...
    if isinstance(model, nn.DataParallel):
        t_h = model.module.h.shape[0]
        a = torch.matmul(p,model.module.h.permute(1,0)) / model.module.sqrtd # 10 x 8 x 8
        a = nn.functional.softmax(a,dim=2)
        p_h = p.reshape(sq,t_p,1,model.module.feat_dim) - model.module.h.reshape(1,1,t_h,model.module.feat_dim)
        q = torch.einsum('nij,nijc->njc',a,p_h)
        q_norm = nn.functional.normalize(q,p=2,dim=2).reshape(sq,t*model.module.feat_dim)
        return model.module.fc(q_norm) # 10 x 2048

    else:
        t_h = model.h.shape[0]
        a = torch.matmul(p,model.h.permute(1,0)) / model.sqrtd # 10 x 8 x 8
        a = nn.functional.softmax(a,dim=2)
        p_h = p.reshape(sq,t_p,1,model.feat_dim) - model.h.reshape(1,1,t_h,model.feat_dim)
        q = torch.einsum('nij,nijc->njc',a,p_h)
        q_norm = nn.functional.normalize(q,p=2,dim=2).reshape(sq,t*model.feat_dim)
        return model.fc(q_norm) # 10 x 2048
...
if __name__=='__main__':
    ...
    if params.method in ['cmn']:
        ...
        model = CMN( model_dict[params.model], **train_few_shot_params )
    else:
